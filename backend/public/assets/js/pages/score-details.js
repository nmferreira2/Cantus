...
function render(score) {
    ...
    document.querySelector("#score-detail").innerHTML = `
        <section class="page-heading"><div><a href="/scores" class="back-link" data-link><i class="bi bi-arrow-left"></i> Partituras</a><div class="d-flex gap-2 mt-3 mb-2"><span class="type-badge">${score.format === "MUSICXML" ? "MusicXML" : "PDF"}</span>${statusBadge(score.active)}</div><h2>${escapeHtml(score.title)}</h2><p class="page-description">Para <a href="/songs/${encodeURIComponent(score.song.id)}" data-link>${escapeHtml(score.song.title)}</a></p></div><div class="d-flex gap-2"><a href="/scores/${encodeURIComponent(score.id)}/edit" class="btn btn-primary" data-link><i class="bi bi-pencil"></i> Editar</a><button id="archive-score" class="btn btn-light text-danger"><i class="bi bi-archive"></i> Arquivar</button></div></section>
...
function bind(score) {
    document.querySelector("#archive-score").addEventListener("click", async (event) => {
        ...
        try { await archiveScore(score.id); setFlash("Partitura arquivada."); router.navigate("/scores?status=archived"); }
    ...
    document.querySelector("#score-version-form").addEventListener("submit", async (event) => {
