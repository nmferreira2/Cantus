...
function render(score) {
    ...
    document.querySelector("#score-detail").innerHTML = `
        <section class="page-heading"><div><a href="/songs/${encodeURIComponent(score.song.id)}" class="back-link" data-link><i class="bi bi-arrow-left"></i> ${escapeHtml(score.song.title)}</a><div class="d-flex gap-2 mt-3 mb-2"><span class="type-badge">${score.format === "MUSICXML" ? "MusicXML" : "PDF"}</span>${statusBadge(score.active)}</div><h2>${escapeHtml(score.title)}</h2><p class="page-description">Para <a href="/songs/${encodeURIComponent(score.song.id)}" data-link>${escapeHtml(score.song.title)}</a><br>${escapeHtml(authorship)}</p></div><div class="d-flex gap-2"><a href="/scores/${encodeURIComponent(score.id)}/edit" class="btn btn-primary" data-link><i class="bi bi-pencil"></i> Editar</a><button id="archive-score" class="btn btn-light text-danger"><i class="bi bi-archive"></i> Arquivar</button></div></section>
