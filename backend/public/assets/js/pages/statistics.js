...
function render(data) {
    document.querySelector("#statistics-content").innerHTML = `
        ...
        <div class="statistics-grid">
            ...
            ${chartCard("Cânticos por idioma", data.charts.songsByLanguage)}
            ${chartCard("Missas por mês", data.charts.massesPerMonth)}
            <section class="card-surface statistics-list"><div class="card-heading"><span class="card-heading-icon"><i class="bi bi-trophy"></i></span><div><h3>Cânticos mais utilizados</h3><p>Nos planeamentos de missas</p></div></div>${data.mostUsedSongs.length ? data.mostUsedSongs.map((song, index) => `<a href="/songs/${encodeURIComponent(song.id)}" class="ranking-row" data-link><span>${index + 1}</span><div><strong>${escapeHtml(song.title)}</strong><small>${songTypeLabel(song.songType)}</small></div><b>${song.uses}</b></a>`).join("") : '<p class="attachment-empty">A utilização aparecerá após o planeamento de missas.</p>'}</section>
